...
5. **Regenerate the derived artifacts.** Editing anything under `shell/`, `knowledgebase/`, or `README.md` changes the CLI index. Run `npm run build:cli-index` and commit `assets/cli-index.json` with the content change; `npm test` fails on any drift. The pre-commit hook rebuilds and stages `assets/search-index.json` for you.
6. **Validate before submitting.** Check the page you changed first, then run the complete suite:
   ...
   `npm test` runs the lint, link, spell, snippet self-test, and CLI-index gates in that order, and the pre-commit hook applies the lint, spell, and link checks to staged files automatically. The snippet self-test has fixtures asserting against real lines in `shell/git.md`, `shell/docker.md`, `shell/jq.md`, and `shell/linux.md`, so editing those pages can fail the gate; update the assertion in `scripts/build-snippets.mjs` rather than deleting it.
...
7. Update this note and the [README](README.md) CLI section to describe `npx ucheat` as available rather than planned.
