...
- Node.js **22.22.1 or newer** with npm, for the validation tooling and the exports — `cspell` and `lint-staged` set that floor, so `npm test` and the pre-commit hook will not run below it
- Node.js 20 or newer to run the CLI itself, which is what the `engines` field and the badge above declare; its runtime dependencies need no more than that
...
Commit the regenerated `assets/cli-index.json` alongside the content change — `npm test` fails on any drift. The same applies to `assets/search-index.json`: the pre-commit hook rebuilds and stages it for you, and `npm test` fails if a committed index is stale.
...
That runs six gates in order: `lint:md`, `check:links`, `spell`, `check:snippets`, `check:cli-index`, `check:index`.

`check:links` validates relative links and in-page anchors only, so the gate makes no network calls and cannot fail because a third-party site is down. External URLs are checked by `npm run check:links:external`, which runs weekly rather than on every push.

The lint, link, and spell scripts enumerate **tracked** Markdown with `git ls-files`, so a brand-new page is not checked until it has been `git add`ed once. There is no single-file runner; to check one file, call the tool directly:
